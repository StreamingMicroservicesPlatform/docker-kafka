...
COPY configs/kafka-configs /home/nonroot/native-config
...
WORKDIR /usr/local
COPY --from=0 /home/nonroot/kafka-configs ./bin/kafka-configs.sh

ENTRYPOINT [ "/usr/local/bin/kafka-configs.sh" ]
