...
docker run --rm --network="native-usecases" \
  -v $(configvolume zookeeper-server-start):/home/nonroot/native-config \
  -v ${basedir}/native-usecases.sh.configs:/etc/kafka \
  --name zoo-0 --expose=2181 solsson/kafka:nativeagent-zookeeper-server-start \
  /etc/kafka/zookeeper.properties.zoo-0

sleep 1

docker run --rm -d --network="native-usecases" \
  -v $(configvolume zookeeper-server-start):/home/nonroot/native-config \
  -v ${basedir}/native-usecases.sh.configs:/etc/kafka \
  --name zoo-1 --expose=2181 solsson/kafka:nativeagent-zookeeper-server-start \
  /etc/kafka/zookeeper.properties.zoo-0

sleep 1
echo "TEMP"; exit 0

docker run --rm -d --network="native-usecases" -v $(configvolume kafka-server-start):/home/nonroot/native-config \
  --name kafka-0 --expose=9092 solsson/kafka:nativeagent-kafka-server-start \
  /etc/kafka/server.properties \
  --override zookeeper.connect=zoo-0:2181

docker run --rm --network="native-usecases" -v $(configvolume kafka-topics):/home/nonroot/native-config \
