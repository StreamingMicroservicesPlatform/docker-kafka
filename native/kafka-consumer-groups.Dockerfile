...
RUN native-image \
  ...
  /home/nonroot/kafka-consumer-groups
...
WORKDIR /usr/local
COPY --from=0 /home/nonroot/kafka-consumer-groups ./bin/kafka-consumer-groups.sh

ENTRYPOINT [ "/usr/local/bin/kafka-consumer-groups.sh" ]
